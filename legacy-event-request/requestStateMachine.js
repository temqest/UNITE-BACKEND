  ...
  async canPerformAction(state, userId, action, request = {}) {
    ...
    if (!hasPermission) {
      ...
      try {
        ...
        userAuthority = actor?.authority || await authorityService.calculateUserAuthority(userId);
      } catch (e) {
      ...
      console.warn('[PERMISSION DENIED]', {
        ...
        requiredPermission: `${requiredPerm.resource}.${requiredPerm.action}`,
        requestAction: action,
        requestState: state,
        locationId: locationId?.toString() || null,
        requestId: request._id?.toString() || request.Request_ID || null,
        userAuthority,
    ...
    const approveActions = [ACTIONS.CONFIRM];
    ...
    if (reviewActions.includes(action) || approveActions.includes(action)) {
      ...
      if (!isSystemAdmin && requesterAuthority > actorAuthority) {
        console.warn('[AUTHORITY DENIED]', {
          userId: userId.toString(),
          actorAuthority,
          requesterId: requesterIdToCheck?.toString() || null,
          requesterAuthority,
          requestAction: action,
          requestState: state,
          requestId: request._id?.toString() || request.Request_ID || null,
          reason: 'INSUFFICIENT_AUTHORITY',
          message: `Actor authority (${actorAuthority}) is lower than requester authority (${requesterAuthority})`,
          timestamp: new Date().toISOString()
        });
        return false;
      }
      ...
      if (isSystemAdmin && requesterAuthority > actorAuthority) {
        console.warn('[ADMIN OVERRIDE]', {
          ...
          message: `System admin overriding authority hierarchy check`,
          timestamp: new Date().toISOString()
        });
      }
    }
