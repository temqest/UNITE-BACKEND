...
function getHumanStatusLabel(status, request = {}) {
  ...
  // Map canonical statuses to human-readable labels
  
  // Pending states
  if (s === 'pending-review' || s.includes('pending')) {
    return 'Waiting for Review';
  }

  // Review states
  if (s === 'review-accepted') {
  ...
  if (s === 'review-rescheduled' || s.includes('resched') || s.includes('reschedule') || s.includes('rescheduled')) {
    return 'Reschedule Requested';
  }
  
  if (s.includes('review') && !s.includes('accepted') && !s.includes('rejected') && !s.includes('resched')) {
    return 'Under Review';
  }
  ...
  if (s.includes('pending_admin_review') || s.includes('pending_coordinator_review') || s.includes('pending_stakeholder_review')) {
    return 'Waiting for Review';
  }
  
  if (s.includes('accepted_by') || s.includes('accepted-by')) {
    return 'Accepted / Confirmed';
  }
  
  if (s.includes('rescheduled_by') || s.includes('rescheduled-by')) {
    return 'Reschedule Requested';
  }
