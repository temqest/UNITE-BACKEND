...
const { User, EventRequest } = require('../../models');
...
class EventRequestController {
  ...
  async createEventRequest(req, res) {
    try {
      ...
      if (user) {
        ...
        // Auto-resolve coordinator if not provided (permission-based: users with request.create can create)
        // If coordinator is not provided, try to auto-resolve based on organization + coverage area
        if (!coordinatorId) {
          console.log('[createEventRequest] Coordinator not provided - attempting auto-resolution based on organization + coverage');
          
          // For users creating requests, set stakeholder reference if applicable
          // This is determined by business rules, not authority level
          try {
            const { User } = require('../../models/index');
            const requester = await User.findById(actorId) || await User.findByLegacyId(actorId);
            
            if (requester && requester.locations && requester.locations.municipalityId) {
              // Set stakeholder reference if requester is creating on their own behalf
              // This is a business rule, not an authorization check
              eventData.stakeholder_id = actorId;
              eventData.Stakeholder_ID = actorId;
              eventData.MadeByStakeholderID = actorId;
              
              // Get requester's organization and municipality
                ...
                const requesterMunicipalityId = requester.locations.municipalityId.toString();
                
                // Find matching coordinator based on organization + coverage area
                // Use permission service to find users with coordinator-level permissions
                const permissionService = require('../../services/users_services/permission.service');
                const { AUTHORITY_TIERS } = require('../../services/users_services/authority.service');
                
                // Find coordinators (users with authority >= COORDINATOR and request.review permission)
                ...
                for (const coordinator of coordinators) {
                  ...
                  let orgMatch = false;
                  if (requesterOrgIds.size > 0 && coordinatorOrgIds.size > 0) {
                    for (const requesterOrgId of requesterOrgIds) {
                      if (coordinatorOrgIds.has(requesterOrgId)) {
                        orgMatch = true;
                        break;
                      }
                    }
                  ...
                  if (!orgMatch && requesterOrgIds.size > 0) {
                    continue;
                  }
                  ...
                  if (coordinatorMunicipalityIds.has(requesterMunicipalityId)) {
          ...
          // If coordinatorId is still missing, use authenticated user's id as fallback
          if (!coordinatorId && actorId) {
            coordinatorId = actorId;
          }

          // If stakeholder_id is provided in eventData, use it
  ...
  async createImmediateEvent(req, res) {
    try {
      ...
      // MODERNIZED: Use permission-based authorization instead of authority tier checks
      const permissionService = require('../../services/users_services/permission.service');
      const userAuthority = userDoc.authority || 20;
      const locationId = body.locationId || body.district;
      
      // Check if user has permission to create immediate events
      ...
      if (!hasCreatePermission) {
        console.log(`[createImmediateEvent] DENIED - Missing event.create permission`, {
        ...
        return res.status(403).json({
          success: false,
          message: 'Not authorized to create immediate events'
        });
      }
      
      // Check if user has permission to create events for other coordinators (admin-level permission)
      const canCreateForOthers = await permissionService.checkPermission(
        creatorId,
        'event',
        'createForOthers',
        { locationId }
      ) || userAuthority >= AUTHORITY_TIERS.OPERATIONAL_ADMIN; // System admins can always create for others
      
      // LOCK: Users without createForOthers permission cannot change coordinator field
      if (!canCreateForOthers) {
        console.log(`[createImmediateEvent] LOCK applied - User (authority ${userAuthority}) restricted to self`, {
          creatorId: userDoc._id,
        ...
        console.log(`[createImmediateEvent] UNLOCK - User (authority ${userAuthority}) can select coordinator`, {
          creatorId: userDoc._id,
          selectedCoordinator: body.coordinator_id
        });
      }

      // RESTRICT: Users without createForOthers permission can only select stakeholders within their jurisdiction
      if (!canCreateForOthers && body.stakeholder_id) {
      ...
      } else if (body.stakeholder_id && canCreateForOthers) {
        console.log(`[createImmediateEvent] NO RESTRICTION - User with createForOthers permission can select any stakeholder`, {
          creatorId: userDoc._id,
          selectedStakeholder: body.stakeholder_id
        });
      }

      // Map creatorRole for service (normalize) - use authority for role snapshot only
      // AUTHORITY_TIERS already declared above (line 234)
      if (!creatorRole) {
        if (userAuthority >= AUTHORITY_TIERS.OPERATIONAL_ADMIN) {
          creatorRole = 'system-admin';
        } else if (userAuthority >= AUTHORITY_TIERS.COORDINATOR) {
  ...
  async getMyRequests(req, res) {
    try {
      ...
      const enriched = await Promise.all(result.requests.map(async (r) => {
        ...
        // Use User model instead of legacy models
        // Prefer new assignedCoordinator field, fallback to legacy coordinator_id
        let coordinatorUser = null;
        const coordinatorId = r.assignedCoordinator?.userId || r.assignedCoordinator?.id || r.coordinator_id;
        if (coordinatorId) {
          if (require('mongoose').Types.ObjectId.isValid(coordinatorId)) {
            coordinatorUser = await User.findById(coordinatorId).catch(() => null);
          } else {
            coordinatorUser = await User.findByLegacyId(coordinatorId).catch(() => null);
          }
        }

        // Fetch stakeholder info if request has stakeholder reference
        // Prefer new stakeholderReference field, fallback to legacy stakeholder_id
        let stakeholderUser = null;
        const stakeholderId = r.stakeholderReference?.userId || r.stakeholderReference?.id || r.stakeholder_id;
        if (stakeholderId) {
          if (require('mongoose').Types.ObjectId.isValid(stakeholderId)) {
            stakeholderUser = await User.findById(stakeholderId).catch(() => null);
          } else {
            stakeholderUser = await User.findByLegacyId(stakeholderId).catch(() => null);
          }
        }
        ...
        // Get human-readable status label (generic, scalable)
        const { getHumanStatusLabel } = require('../../services/request_services/requestFlow.helpers');
        const statusLabel = getHumanStatusLabel(r.Status || r.status, r);
        ...
        const plain = {
          ...
          statusLabel // Add generic status label for frontend
        };
        try {
          if (stakeholderUser) {
            plain.createdByName = stakeholderUser.fullName || `${stakeholderUser.firstName} ${stakeholderUser.lastName}`;
  ...
  async reviewDecision(req, res) {
    try {
      ...
      // Use new requester field, fallback to legacy made_by_id
      const requesterId = request.requester?.userId || request.made_by_id;
      const requester = requesterId ? 
        await User.findById(requesterId).select('authority') : null;
      ...
      const requesterAuthority = requester?.authority || (request.requester?.authoritySnapshot || 20);
      const isSystemAdmin = reviewerAuthority >= 100;
      
      // Validate actor is the assigned reviewer (unless system admin)
      if (!isSystemAdmin) {
        const reviewerId = request.reviewer?.userId || request.reviewer?.id;
        if (reviewerId && reviewerId.toString() !== user.id.toString()) {
          return res.status(403).json({
            success: false,
